		...
		function getStore(modules) {
			...
			let neededObjects = [
				...
				},
				{
					id: "Presence",
					conditions: value =>
						value.default && value.default.Presence ? value.default : null
				},
				{
					id: "WapDelete",
					conditions: module =>
			...
			window.Store.Chat.modelClass.prototype.sendMessage = function () {
...
//Check
window.WAPI.getContactForPhone = async function (id) {
	try {
		let id2 = id.match(/\d+/g).join("") + "@c.us";

		let phone = await window.Store.WapQuery.queryExist(id2);

		if (phone.jid) return phone.jid;
		...
		phone = await window.WAPI.getGroupMetadata(groupId);

		if (phone) return phone.id;

		console.log("getContactForPhone: " + JSON.stringify(phone));
		return null;
	} catch (error) {
		console.log("getContactForPhone - catch: " + JSON.stringify(error));
...
window.WAPI.sendMessage = async function (id, message) {
	...
	if ((!chat && !id.includes("g")) || chat.msgs.models.length == 0) {
		let contact = WAPI.getContact(id);
	...
	if (chat !== undefined) {
		return await chat
			.sendMessage(message)
			.then(() => chat.lastReceivedKey._serialized);
	}
...
//Check
/**
 * Registers a callback to be called when logged status changes.
 * @param callback - function - Callback function to be called when the logged status changes. this returns 'true' or 'false'
 * @returns {boolean}
 */
window.WAPI.onConnected = function (callback) {
	window.Store.Conn.on("change:connected", ({ connected }) =>
		callback(connected)
	);
 ...
 */
WAPI.onGlobalParticipantsChanged = function (callback) {
...
window.WAPI.onParticipantsChanged = function (groupId, callback) {
	...
	chat.groupMetadata.participants.on("all", (eventName, eventData, extra) => {
		if (events.includes(eventName)) {
			let action = eventName;
			if (eventName == "change:isAdmin") {
				action = extra ? subtypeEvents[4] : subtypeEvents[5];
			}
			callback({
				by: undefined,
				action: action,
...
window.WAPI._onParticipantsChanged = function (groupId, callback) {
	...
	if (!groupParticipantsEvents[groupId]) {
		groupParticipantsEvents[groupId] = {};
		metadata.participants.forEach(participant => {
			groupParticipantsEvents[groupId][participant.id.toString()] = {
				subtype: "add",
				from: metadata.owner
			};
		});
	...
	chat.on("change:groupMetadata.participants", _ =>
		chat.on("all", (x, y) => {
			...
			if (
				...
				if (
					groupParticipantsEvents[groupId][rec] &&
					groupParticipantsEvents[groupId][recipients[0]].subtype == subtype
				) {
					...
					if (i == 0) {
						...
						groupParticipantsEvents[groupId][rec] = { subtype, author };
						//fire the callback
...
window.WAPI.joinGroupViaLink = async function (link) {
	return await Store.WapQuery.acceptGroupInvite(
		link.split("/").pop()
	).then(res => (res.status === 200 ? res.gid._serialized : res.status));
	let code = link;
...
window.WAPI.sendVideoAsGif = async function (
	...
	return await Store.Chat.find(chatId).then(async chat => {
		...
		new Store.MediaCollection(chat);
 ...
 *
 * @param {string} chatId '000000000000@c.us'
 * @param {boolean} on true to turn on simulated typing, false to turn it off //you need to manually turn this off.
 */
...
window.WAPI._STICKERDUMP = async function (chatId) {
	...
	return await Promise.race(
		Store.StickerPack.models.forEach(pack =>
			pack.stickers
				.fetch()
				.then(_ => pack.stickers.models.forEach(stkr => stkr.sendToChat(chat)))
		)
	).catch(e => {
		console.log(e);
	});
